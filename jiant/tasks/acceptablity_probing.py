...
@register_task("npi_sup", rel_path="NPI/splits/superlative")
@register_task("all_npi", rel_path="NPI/combs/all_env")
@register_task("wilcox_npi", rel_path="NPI/wilcox")
@register_task("hd_npi_adv", rel_path="NPI/combs/minus_adverbs")
@register_task("hd_npi_cond", rel_path="NPI/combs/minus_conditionals")
@register_task("hd_npi_negdet", rel_path="NPI/combs/minus_determiner_negation_biclausal")
@register_task("hd_npi_negsent", rel_path="NPI/combs/minus_sentential_negation_biclausal")
@register_task("hd_npi_only", rel_path="NPI/combs/minus_only")
@register_task("hd_npi_ques", rel_path="NPI/combs/minus_questions")
@register_task("hd_npi_quessmp", rel_path="NPI/combs/minus_simplequestions")
@register_task("hd_npi_qnt", rel_path="NPI/combs/minus_quantifiers")
@register_task("hd_npi_sup", rel_path="NPI/combs/minus_superlative")
...
class NPIClozePairTask(PairClassificationTask):
    ...
    def process_split(self, split, indexers, model_preprocessing_interface):
        def _make_instance(input1, input2, labels, tagids):
            ...
            d = {}
            d["input1"] = sentence_to_text_field(
                model_preprocessing_interface.lm_boundary_token_fn(input1), indexers
            )
            d["sent1_str"] = MetadataField(" ".join(input1))
            d["input2"] = sentence_to_text_field(
                model_preprocessing_interface.lm_boundary_token_fn(input2), indexers
            )
            ...
            d["input0"] = sentence_to_text_field(
...
class BlimpOnePrefixLMTask(BlimpTask):
    ...
    def load_data(self):
        ...
        for example in data:
            ...
            good_words.append(
                tokenize_and_truncate(
                    self._tokenizer_name, example["one_prefix_word_good"], self.max_seq_len
                )
            )
            bad_words.append(
                tokenize_and_truncate(
    ...
    def process_split(self, split, indexers, model_preprocessing_interface):
        def _make_instance(
            ...
            input2 = model_preprocessing_interface.lm_boundary_token_fn(sent2)
            ...
            d["shared_prefix_length"] = IndexField(
                common_prefix_length(
                    model_preprocessing_interface.lm_boundary_token_fn(shared_prefix), input1
                ),
                d["input1"],
            )
...
class BlimpTwoPrefixLMTask(BlimpTask):
    ...
    def load_data(self):
        ...
        for example in data:
            ...
            bad_prefixes.append(
                tokenize_and_truncate(
                    self._tokenizer_name, example["two_prefix_prefix_bad"], self.max_seq_len
                )
            )
            shared_words.append(
                tokenize_and_truncate(
    ...
    def process_split(self, split, indexers, model_preprocessing_interface):
        def _make_instance(
            ...
            d["good_prefix_length"] = IndexField(
                common_prefix_length(
                    model_preprocessing_interface.lm_boundary_token_fn(good_prefix), input1
                ),
                d["input1"],
            )
            ...
            d["bad_prefix_length"] = IndexField(
                common_prefix_length(
                    model_preprocessing_interface.lm_boundary_token_fn(bad_prefix), input2
                ),
                d["input2"],
            )
...
class BlimpAcceptabilityTask(SingleClassificationTask):
    ...
    def load_data(self):
        """Load the data"""
        self.train_data_text = self.load_split("train.jsonl")
        self.test_data_text = self.load_split("test.jsonl")
...
class BlimpProbeTask(SingleClassificationTask):
    ...
    def load_split(self, file_name):
        ...
        for example in data:
            ...
            if self.target == "good":
                # Currently, the good feature corresponds exactly to the task.
                # is there a way we can break out of this?
                labels.append(1)
                labels.append(0)
            else:
                # TODO: Double check logic.
                # {
                #     "sentence_good": "The shows about Spain do stun most girls.",
                #     "sentence_bad": "The shows about Spain does stun most girls.",
                #     "co-occurs": false,
                #     "UID": "distractor_agreement_relational_noun-distractor_agreement_relational_noun_probing-0.5",
                # }
                # {
                #     "sentence_good": "A mother of Galileo hasn't hidden a sweater.",
                #     "sentence_bad": "A mother of Galileo haven't hidden a sweater.",
                #     "co-occurs": true,
                #     "UID": "distractor_agreement_relational_noun-distractor_agreement_relational_noun_probing-0.5",
                # }

                label = 1 if example["co-occurs"] else 0
                labels.append(label)
                label = 0 if example["co-occurs"] else 1
                labels.append(label)
...
@register_task(
    ...
    linguistic_property="sentential_negation_npi_scope_probing",
    rate="0.5",
    target="good",
    rel_path="blimp/probing/",
)
...
@register_task(
    "sentential_negation_npi_scope_probing-0.8-bad",
    linguistic_property="sentential_negation_npi_scope_probing",
    rate="0.8",
    target="bad",
    rel_path="blimp/probing/",
...
class BlimpNPIProbeTask(SingleClassificationTask):
    def __init__(self, path, max_seq_len, name, linguistic_property, rate, target, **kw):
        """ """
        super(BlimpNPIProbeTask, self).__init__(name, n_classes=2, **kw)
        self.path = path
        self.max_seq_len = max_seq_len
    ...
    def load_split(self, file_name):
        ...
        for example in data:
            ...
            if self.target == "good":
                ...
                # This is when there will be negated scope
                labels.append(1 if example["label"] == "good" else 0)
            else:
                # TODO: Double check logic.
                # These are the cases where there will be a negation
                labels.append(1 if example["negated_outside_scope"] == 1 or example["label"] == "good" else 0)
